...
the human runs `/arch-gsd:verify-phase N` and the pipeline validates design completeness
autonomously.
...
Extract the phase number from $ARGUMENTS. If $ARGUMENTS is empty, display:
  ...
  "Usage: /arch-gsd:verify-phase [phase-number]
  Example: /arch-gsd:verify-phase 3"
...
If design/ directory does not exist or is empty:
  Display:
  "Phase {N} design documents not found. Run /arch-gsd:execute-phase {N} first.
...
If STATE.md shows Phase N as not started or in-progress with no design documents:
  Display:
  "Phase {N} execution appears incomplete. Run /arch-gsd:execute-phase {N} first to produce
...
**If status is "failed":**
  ...
  Fix: Run /arch-gsd:execute-phase {N} to produce design documents first."
...
**If status is "human_needed":**
  ...
  To proceed:
    ...
    3. Update the affected design documents via /arch-gsd:execute-phase {N} --correction
    4. Re-run /arch-gsd:verify-phase {N}"
  STOP — do not proceed to arch-integrator.
...
**If status is "human_needed":**
  ...
  To proceed:
    ...
    3. Re-run /arch-gsd:verify-phase {N}"
  STOP — do not proceed to MANIFEST.md generation.
...
Verification gaps detected ({findings_count}):
  ...
  Recommended: Run /arch-gsd:execute-phase {N} in correction mode to close gaps,
  then re-run /arch-gsd:verify-phase {N}

{If arch_integrator_status is "gaps_found":}
Integration gaps detected ({gaps_count}):
  {list gap summaries from INTEGRATION-REPORT.md}
  Recommended: Resolve cross-phase reference issues, then re-run /arch-gsd:verify-phase {N}
