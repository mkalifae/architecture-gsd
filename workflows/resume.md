...
This workflow satisfies STAT-05: after any session ends, the user runs /arch-gsd:resume and
...
If neither file exists, display:
  ...
  Run /arch-gsd:new-system first to initialize a project."
...
If ROADMAP.md is missing, note "ROADMAP.md not yet generated — run /arch-gsd:execute-phase 1
...
If "Resume with:" field is present and non-empty:
  ...
  Also available:
    /arch-gsd:progress   — view status without continuing
    /arch-gsd:verify-phase [N]   — run verification if phase execution is complete
  ```

If "Resume with:" field is missing or empty, fall back to determining next action from
STATE.md Current Position:

  - If Status is "Phase N complete": suggest /arch-gsd:execute-phase N+1
  - If Status is "Phase N verified": suggest /arch-gsd:execute-phase N+1
  - If Status shows a phase in progress: suggest /arch-gsd:execute-phase N (to continue)
  - If Status is the initial state (no phase started): suggest /arch-gsd:execute-phase 1
...
- [ ] No .continue-here file logic — arch-gsd uses STATE.md "Resume with:" line
